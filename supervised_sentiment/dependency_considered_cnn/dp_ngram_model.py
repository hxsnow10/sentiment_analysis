...
Input:1. pos tagging input  2. multi_prototype 3. dependency
Model:get sentiment of every n-gram
先做有一个简单的尝试：从denpency tree 提取n-gram:长度(节点数)为n的路径
"""
import logging
import numpy
...
class DenpencyNgramExtracter(Initializable):

    def __init__(self,)

    @application(input='x',output='rval'):
        '''x is tensor3:btach*sen_len*3( word_index, arc_fa_index, arc_type)
        '''
...
class SentiNet(FeedforwardSequence, Initializable):
    ...
    def __init__(self, words_num, vector_len, words_vectors_path, 
        ...
        application_methods = [self.lookup.apply, self.bn1.apply, self.conv.apply, self.flattener.apply, self.bn2.apply,
                               self.top_mlp.apply]
        application_methods = [self.lookup.apply, self.conv.apply, self.flattener.apply,
                               self.top_mlp.apply]
...
def main(save_to='cnn.pkl', num_epochs=10, batch_size=100,
    ...
    _train=H5PYDataset(data_path,which_sets=('train',), subset=slice(0,800))
