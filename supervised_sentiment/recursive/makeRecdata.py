...
def denpency_datamaker(texts=None, grammar_type):
    '''
    input a single text, return  
    X_node: descibe node  
        [sen_len, index0, ...], 
        [sen_len, pos_tag0, ...]
    X_tree/graph: describe graph
        grammar_type='denpency':
        [len, arctype0, ...], 
        [len, arcfrom_id, ...]
        grammar_type='constituency':
        [sen_len,
...
def chuncked_create_hdf5(process,input_path , output_path , 
    ...
    for name,shape in process().iteritems():
        print name,shape
        f.create_dataset(name, (10000, shape[1]), maxshape=(None, shape[1]), dtype='int32')
    def textss(path,n):
        ii=open(path,'r')
