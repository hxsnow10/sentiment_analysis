...
from blocks.algorithms import GradientDescent, Scale, AdaDelta, Adam, CompositeRule, StepClipping
...
class SentiNet(Initializable):
    ...
    def __init__(self, words_num, vector_len, words_vectors_path, 
        ...
        self.bdir= Bidirectional(
                    prototype=GatedRecurrent(dim=100),weights_init=IsotropicGaussian(),biases_init=Constant(1))
        ...
        self.children=[self.lookup, self.bdir, self.flattener, self.top_mlp]

    @application(input='inputs',output='rval')
    def apply(self, inputs):
        inputs=self.lookup.apply(inputs)
        gate_inputs=tensor.repeat(inputs, 2, axis=2)
        inputs=self.bdir.apply(inputs=inputs,gate_inputs=gate_inputs)
        print 'tttttttttttttttttttttttttt',type(inputs)
...
def main(save_to='rnn.pkl', num_epochs=50, batch_size=50,
    ...
    '''
    _train=H5PYDataset(data_path,which_sets=('train',),subset=slice(0,10000))
