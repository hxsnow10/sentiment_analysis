        ...
        self.children=\
            [FeedforwardSequence(
                application_methods=[Convolutional(filter_size=filter_sizes[i],
                        ...
                        simple_transformer(fn=lambda x:x[:,:,:,0].dimshuffle(0,2,1)).apply,
                        MLP([Identity()], [num_filters[i],2], weights_init=IsotropicGaussian(),
                                            biases_init=Constant(1),name='linear_{}'.format(i)).apply,
                        ])
             for i in range(len(filter_sizes))]
        self.children.append(NDimensionalSoftmax())
        print self.children
...
class SentiNet(FeedforwardSequence, Initializable):
    ...
    def __init__(self, words_num, vector_len, words_vectors_path, 
        ...
        application_methods = [self.lookup.apply, self.conv.apply]
