...
class Multi_Lookup(Initializable):
    ...
    def apply(self, cen, con):
        '''cen: b*sen_len, con:b*sen_len*wondow_size
           return b*sen_len*n
        '''
        con=self.Context_Vectors[con]#B*sen_len*n
        cen=self.Meaning_Vectors[cen]
        con=con.sum(axis=1)#B*sen_len*w_size*n
        index=tensor.argmax(tensor.batched_tensordot(con.reshape(),cen.reshape(),[[1],[2]]).reshape(),2)#TODO reshape
        rval=cen[index]#TODO
        return rval
