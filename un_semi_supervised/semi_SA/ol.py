...
if __name__=='__main__':
    vocab=get_vocab('chara.txt')
    ii=open('data_c','r')
    X,y=[],[]
    for i  in ii:
        label,sent=i.strip().split('\t')
        if label not in ['-1','0','1']:continue
        y.append(label)
        X.append('\t'.join([i.encode('utf-8') for i in sent.decode('utf-8')]))
    Z=zip(X,y)
    random.shuffle(Z)
    X,y=zip(*Z)
    X,y=IndexSequenceTransformer(sen_len=100,words=vocab).transform(X,y,split='\t')#允许词典中有非极性词，在分词的时候把非极性词过滤
