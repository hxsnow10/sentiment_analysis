...
dict based means that we have label data about 1. cluster nums 2. center of meaning
本来的聚类问题，现在变成了一个半监督训练的问题。
'''
import logging
import numpy
...
def main(save_to='cluster.pkl', num_epochs=200, batch_size=100,
        data_path='data.hdf5',
        vec_path='vec2.txt', sen_len=25,window_size=5,prototype_num=3,# config LookUp_Ruled
        return_model=False
        ):
    ...
    checkpoint.add_condition(['after_epoch'],
                                     predicate=OnLogRecord('test_kmeans_cost_output_0'))
    extensions = [Timing(),
