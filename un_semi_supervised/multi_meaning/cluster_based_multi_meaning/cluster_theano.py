...
class Kmeans(Initializable):
    ...
    def cost(self, state_blow):
        ...
        label=T.set_subtensor(state_blow[:,1:], T.argmin(norm,2)*mask)
        cost=T.sum(dist)
        return cost,label
    
def train(save_to='cluster.pkl', num_epochs=200, batch_size=100,
    ...
    _test=H5PYDataset(data_path,which_sets=('develop',),subset=slice(800,900))
    ...
    algorithm = GradientDescent(
        ...
        step_rule=AdaDelta())#IN FACT, i find i learns too slow
    # algorithm.add_updates(extra_updates)
    # `Timing` extension reports time for reading data, aggregating a batch
    # and monitoring;
    # `ProgressBar` displays a nice progress bar during training.
     
    checkpoint = Checkpoint(save_to, after_training=False)#TODO: only save parameters
